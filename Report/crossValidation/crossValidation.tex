...
Cross validation is evaluation of model. The cross validation is better then residuals. The reason is that with the residuals they do not give an indication how well the model will predict with the new data that it did not sow. One way to avoid this problem is to divide the data in several peace, in this case some data can be removed from training set to validate the model. This is the basic idea of cross validation. There is several possibilities how can we perform cross validation some of them will be described below.
...
23.36190289258723
\end{lstlisting}
We can reduce the MSE polynomial or cubic regression.
\begin{lstlisting}[language=Python]
